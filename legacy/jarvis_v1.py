...
# le PC (ask_claude_with_tools), là où la fiabilité du tool-use compte vraiment.
# Si Ollama est injoignable ou échoue avant toute parole, repli automatique sur
# Claude (voir ask_chat_stream) : la conversation ne reste jamais muette.
...
# Profils de barge-in (interruption pendant que Jarvis parle) selon la sortie audio.
# Au casque il n'y a aucune boucle acoustique : on peut être sensible.
# Sur haut-parleurs le micro réentend Jarvis — sans annulation d'écho il faut être
# nettement plus strict, sinon il s'interrompt lui-même en permanence.
INTERRUPT_PROFILES = {
...
_HEADSET_HINTS = ("casque", "ecouteur", "headset", "headphone", "earphone", "earbud",
                  "airpod", "arctis", "hyperx", "steelseries", "jabra", "bose",
                  "sennheiser", "wh-1000", "buds", "quietcomfort")
_SPEAKER_HINTS = ("haut-parleur", "hautparleur", "speaker", "hdmi", "display",
...
# Filtre « y a-t-il du son ? » avant de scorer. Depuis la normalisation il ne sert
# plus qu'à éviter de scorer du silence — l'ancienne valeur (0.015) était au-dessus
# du niveau réel du micro (parole mesurée à RMS ~0.005) et bloquait tout.
...
_confirm_request = {"event": threading.Event(), "approved": False}
is_paused = False
is_waiting = True
tray_icon = None
...
def normalize_wake_word_audio(audio):
    ...
    if rms < 1e-7:
        return audio
    return (audio * (WAKE_WORD_TARGET_RMS / rms)).astype(np.float32)
...
def extract_wake_word_features(audio):
    ...
    audio = normalize_wake_word_audio(audio.astype(np.float32))
    mfcc = librosa.feature.mfcc(y=audio, sr=WAKE_WORD_SAMPLE_RATE, n_mfcc=40)
...
# Mots-clés indiquant un fait éphémère (météo, heure) qu'il ne sert à rien de mémoriser.
# Comparés en mots entiers : "satisfait" ou "parfait" ne doivent PAS déclencher "fait".
...
def update_memory():
    ...
    try:
        ...
        if new_data.get("new_facts"):
            memory = load_memory()
            # save_memory() applique déjà le nettoyage (clean_memory_fact) et la
            # troncature finale à 80 — pas besoin d'un second seuil différent ici,
            # qui ne faisait qu'introduire une incohérence silencieuse.
...
def match_mode_trigger(text):
    ...
    normalized = re.sub(rf"^{WAKE_WORD}\b[\s,;:!?-]*", "", normalized).strip()
    if not normalized:
        return None
    for mode in load_modes()["modes"]:
...
def normalize_text_aligned(text):
    """Comme normalize_text mais sans strip : les index restent alignés sur l'original.

    Nécessaire dès qu'on cherche une position dans la version normalisée pour
    ensuite découper la chaîne d'origine (cf. save_explicit_fact).
    """
    if not text:
        return ""
...
def transcribe_stream(stream):
    ...
    L'objectif est que le résultat soit prêt avant la fin de la fenêtre de
    silence (320ms) → latence STT perçue nulle. Ça n'est vrai que si le modèle
    STT répond en moins de ~300ms : c'est le cas de `tiny`, pas de `small`
    sur CPU (cf. STT_MODEL). Le log ci-dessous indique le surcoût réel.
    """
    CHUNK = 512
    sample_rate = 16000
    silent_chunks = 0
...
def _show_confirm_popup(command):
    """Bulle de confirmation Oui/Non pour une commande à risque — thread tkinter dédié."""
    ...
    header = tk.Frame(root, bg="#2e1212", height=42)
    header.pack(fill="x")
    header.pack_propagate(False)
    header.bind("<ButtonPress-1>", start_drag)
    header.bind("<B1-Motion>", do_drag)

    tk.Label(header, text="⚠  JARVIS — Confirmation requise",
             bg="#2e1212", fg="#ff9494",
    ...
    tk.Label(body, text="Action potentiellement irréversible détectée :",
             bg="#0a0a1a", fg="#aac4ee", font=("Segoe UI", 9),
             anchor="w", wraplength=420, justify="left").pack(fill="x", pady=(0, 6))
    tk.Label(body, text=command[:300], bg="#12122e", fg="#ffcf9e",
             font=("Consolas", 9), anchor="w", wraplength=420, justify="left",
             padx=8, pady=8).pack(fill="x", pady=(0, 12))

    def respond(approved):
        _confirm_request["approved"] = approved
        _confirm_request["event"].set()
        root.destroy()

    footer = tk.Frame(root, bg="#0a0a1a")
    footer.pack(fill="x", padx=16, pady=10)
    tk.Button(footer, text="Refuser", command=lambda: respond(False),
              bg="#3a1a1a", fg="#ff9494", font=("Segoe UI", 10, "bold"),
              relief="flat", padx=16, pady=6, cursor="hand2", bd=0).pack(side="left")
    tk.Button(footer, text="Exécuter →", command=lambda: respond(True),
              bg="#2a2a7e", fg="#c8d8ff", font=("Segoe UI", 10, "bold"),
              relief="flat", padx=16, pady=6, cursor="hand2", bd=0).pack(side="right")

    root.protocol("WM_DELETE_WINDOW", lambda: respond(False))
    root.mainloop()

def confirm_destructive_action(command):
    """Bloque jusqu'à la décision de Monsieur. Timeout, fermeture, ou
    interruption vocale déjà en cours → refus par défaut.

    Utilise speak(), pas speak_with_interrupt(). Cette fonction est appelée
    depuis l'intérieur de la boucle d'outils (ask_claude_with_tools), déjà
    couverte par l'écouteur d'interruption démarré par run_and_speak_agent.
    speak_with_interrupt() démarrerait un second écouteur qui lirait le même
    flux micro en parallèle, et son stop_speaking.set() final couperait
    prématurément l'écouteur englobant pour le reste du tour.
    """
    if stop_agent.is_set() or stop_speaking.is_set():
        return False
    _confirm_request["approved"] = False
    _confirm_request["event"].clear()
    threading.Thread(target=_show_confirm_popup, args=(command,), daemon=True).start()
    if not overlay.is_muted():
        speak("Cette action est irréversible, Monsieur. Confirmation requise à l'écran.")
    _confirm_request["event"].wait(timeout=30)
    return _confirm_request["approved"] and not stop_agent.is_set()

def tool_run_powershell(command, force_confirm=False):
    # force_confirm=True quand une donnée non fiable (OCR, contenu de fichier, URL)
    # a été lue plus tôt dans le même tour : une page web ou un document piégé qui
    # contiendrait des instructions ne doit jamais pouvoir déclencher du PowerShell
    # sans qu'un humain ne voie et n'approuve la commande exacte.
    if force_confirm or is_destructive_command(command):
        if not confirm_destructive_action(command):
...
def tool_mouse_click(x, y, button="left"):
    try:
        import pyautogui
        # 0.2s de mouvement "humain" n'a aucune utilité ici (pas de détection à
        # contourner) et ajoute 200ms de latence perçue à chaque clic de l'agent.
...
UNTRUSTED_CONTENT_TOOLS = {"read_screen", "read_file_content", "get_browser_url"}

def wrap_untrusted(content):
    """Encadre du contenu externe (OCR, fichier, URL) pour qu'il ne soit jamais
    confondu avec une instruction de Monsieur.
...
def tool_request_human_help(what_i_tried, what_i_need):
    ...
    Utilise speak(), pas speak_with_interrupt() — même raison que
    confirm_destructive_action : déjà appelée depuis l'intérieur de la boucle
    d'outils, sous l'écoute d'interruption de run_and_speak_agent.
    """
    ...
    if not (stop_agent.is_set() or stop_speaking.is_set()) and not overlay.is_muted():
        speak("Je n'y arrive pas seul, Monsieur. Une fenêtre d'aide vient de s'ouvrir.")
...
def execute_tool(name, args, turn_state=None):
    ...
    if name == "request_human_help":
        ...
        return tool_run_powershell(args.get("command", ""), force_confirm=force)
    elif name == "type_text":
...
def _recent_text_history():
    """Historique récent réduit aux messages texte simples (les blocs outils sont locaux)."""
    return [
...
def _remember_exchange(question, answer):
    conversation_history.append({"role": "user", "content": question})
    conversation_history.append({"role": "assistant", "content": answer})
    if len(conversation_history) > 30:
        conversation_history[:] = conversation_history[-30:]

# Aiguillage conversation vs agent : voir is_pc_command() et run_and_speak_agent().
# Une question conversationnelle n'a aucune raison de transporter les 15
...
def ask_ollama_chat_stream(question):
    """Conversation via un modèle local (Ollama) : gratuit, privé, hors-ligne.

    Même contrat que ask_claude_chat_stream — un générateur qui produit des
    phrases complètes au fil de l'eau — pour brancher exactement la même
    pipeline TTS en streaming, quel que soit le modèle qui répond.

    Lève une exception si Ollama est injoignable ou répond en erreur ;
    c'est à l'appelant (ask_chat_stream) de décider du repli vers Claude.
    """
    static_prompt, dynamic_prompt = get_system_prompt()
    system_text = static_prompt + (f"\n\n{dynamic_prompt}" if dynamic_prompt else "")

    messages = [{"role": "system", "content": system_text}]
    messages += _recent_text_history()
    ...
    print(f"[Ollama] Interrogation de {OLLAMA_MODEL} "
    ...
    resp = requests.post(
        ...
        json={
            ...
            # la génération sur un ramble interminable (rien ne l'arrête avant
            # la fin du contexte). 400 tokens ≈ largement assez pour une
            # réponse Jarvis, qui se veut brève par personnalité.
    ...
    for line in resp.iter_lines():
        ...
        if delta:
            if first_content:
                print(f"[Ollama] Première réponse après {time.time() - t0:.1f}s.")
                first_content = False
            buffer += delta
    ...
    _remember_exchange(question, final)

def ask_chat_stream(question, brain_state=None):
    ...
    pour que l'appelant (l'overlay, notamment) sache qui a réellement répondu.
    """
    ...
    produced_any = False
    try:
        for phrase in ask_ollama_chat_stream(question):
            produced_any = True
            yield phrase
        return
    except Exception as e:
    ...
    yield from ask_claude_chat_stream(question)

def ask_claude_chat_stream(question):
    """Comme ask_claude_chat, mais en streaming : chaque phrase complète est
    produite dès qu'elle est prête. Permet au TTS de commencer à parler la
    première phrase pendant que Claude génère encore la suite, au lieu
    ...
    if not client:
        ...
        yield "Je ne peux pas répondre sans clé API, Monsieur. Vérifiez le fichier .env."
        return
    ...
    try:
        with client.messages.stream(
            ...
            max_tokens=400,
            system=system,
            messages=_recent_text_history() + [{"role": "user", "content": question}]
        ) as stream:
        ...
        track_claude_usage(getattr(final_message, "usage", None))
    except Exception as e:
    ...
    for _ in range(12):
        ...
        if response.stop_reason == "tool_use":
            ...
            for block in response.content:
                ...
                result = execute_tool(block.name, block.input, turn_state)
...
def is_pc_command(question):
    ...
    Sert d'aiguillage : sans outils, le préfixe envoyé au modèle passe
    d'environ 6000 à 1500 tokens pour une simple conversation.
    """
...
def save_explicit_fact(question):
    ...
    for trigger in ["memorise que", "retiens que", "sache que", "note que",
        ...
        # Le fait doit survivre au même filtrage que save_memory appliquera,
        # sinon on confirmerait une mémorisation fantôme.
        fact = clean_memory_fact(fact)
        if not fact:
            return None
        memory = load_memory()
...
def detect_output_profile():
    ...
    if forced in INTERRUPT_PROFILES:
        return forced

    now = time.time()
    if now - _output_profile_cache["checked_at"] < OUTPUT_DEVICE_RECHECK_SECONDS:
    ...
    if any(h in n for h in _HEADSET_HINTS):
        profile = "casque"
    elif any(h in n for h in _SPEAKER_HINTS):
...
def listen_for_interrupt():
    ...
    cfg = INTERRUPT_PROFILES[detect_output_profile()]
...
def run_and_speak_agent(question):
    """Interroge le cerveau adapté et fait parler Jarvis, avec un seul
    listener d'interruption couvrant toute la durée réflexion + parole.

    Pour une question conversationnelle, consomme ask_chat_stream() phrase
    par phrase — modèle local Ollama d'abord (gratuit, privé), repli sur
    Claude si indisponible. La première phrase part au TTS pendant que le
    modèle génère encore la suite, au lieu d'attendre la réponse complète.
    Pour une commande de pilotage PC, Claude reste seul (fiabilité du
    tool-use) et la boucle à outils reste bloquante (les tool_use imbriqués
    rendent le streaming incrémental peu fiable) : la réponse finale est
    prononcée d'un bloc, comme avant.
    ...
    try:
        if is_pc_command(question):
            result = ask_claude_with_tools(question)
            overlay.set_model_label(CLAUDE_MODEL)
            if not (stop_agent.is_set() or stop_speaking.is_set()):
                _say(result)
        else:
            brain_state = {}
            for phrase in ask_chat_stream(question, brain_state):
                if stop_agent.is_set() or stop_speaking.is_set():
                    break
                _say(phrase)
            overlay.set_model_label(
                OLLAMA_MODEL if brain_state.get("source") == "ollama" else CLAUDE_MODEL
            )
    finally:
        stop_speaking.set()  # signal au listener pour qu'il s'arrête proprement
        interrupt_thread.join()
        overlay.set_state("idle")

    if stop_agent.is_set() and not spoken_parts:
        return None
...
def listen_for_wake_word_tflite(stream):
    ...
    while True:
        ...
        if consecutive_hits >= WAKE_WORD_CONSECUTIVE_HITS and now - last_trigger_time > WAKE_WORD_COOLDOWN_SECONDS:
            ...
            if vad_prob < 0.3 and rms < WAKE_WORD_MIN_RMS * 2:
        ...
        while True:
            ...
            if question and len(question) > 2:
                ...
                if direct_response:
                    ...
                    # Tout le reste → Claude (chat en streaming, ou agent à outils)
                    overlay.set_source("ai")
                    print("(Mode Claude)")

                    result = run_and_speak_agent(question)
