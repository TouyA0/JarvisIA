...
| ~~T5~~ | ~~Aucune carte / aucune télécommande visuelle~~ **Résolu** | Nouveau type de carte `tv` (`web/src/components/cards/renderers.jsx`, `Icon.jsx`) : capture/statut en corps de carte + rangée de boutons (`accueil`, `retour`, D-pad, `OK`, `lecture`, volume ±/muet) posés via `card.actions` — `_tv_actions()` dans `brain/tools.py`, même mécanique `/api/tools/execute` que la carte `music`. Émise par `tv_status`, `tv_launch_app` et `tv_screenshot`. | ▲▲ |
...
| ~~T8~~ | ~~Sous-titres & piste audio~~ **Résolu (best effort)** | Pas de commande ADB générique possible (chaque appli a son propre menu) — la description de `tv_screen_dump` dans `brain/tools.py` donne maintenant à Claude la marche à suivre explicite : afficher les contrôles de lecture (`tv_key` DPAD ou `tv_tap` au centre), dumper pour repérer le menu audio/sous-titres, taper dessus, re-dump pour choisir la langue/piste, re-dump pour confirmer. Reste app par app comme prévu, mais Claude n'a plus à le deviner — à vérifier en usage réel sur Netflix/Disney+/Prime | ▲▲ |
...
| ~~T13~~ | ~~Robustesse de la liaison ADB~~ **Résolu** | `_connect()` distingue maintenant port fermé (`ConnectionRefusedError` → message qui pointe vers « Débogage réseau » à réactiver dans Options développeur) d'hôte injoignable (timeout → IP probablement changée). Dans ce second cas, repli sur une découverte mDNS (`_adb-tls-connect._tcp`/`_adb._tcp`, lib `zeroconf`) qui essaie de retrouver le stick sur le réseau local ; l'IP retrouvée est mémorisée (RAM + `data/android_tv_host_cache.json`) pour ne pas resonder à chaque appel. Un thread `android_tv.start_reconnect_loop()` (démarré dans `server.py`, même pattern que `timers.start()`/`proactive.start()`) sonde et reconnecte toutes les 2 min en tâche de fond plutôt que d'attendre la prochaine commande de Monsieur. mDNS reste best effort (best effort de l'appareil à annoncer le service) — l'IP fixe en `.env` demeure la voie normale | ▲▲ |
...
| ~~T15~~ | ~~Aucune confirmation, accès quasi-shell~~ **Résolu** | `_connect_to()` refuse désormais toute IP publique pour l'hôte (fixe ou redécouvert via mDNS) via `_is_lan_host()` — vérifiée avec `ipaddress.ip_address(...).is_global`, échec immédiat (`_PublicHostRefused`, pas de repli mDNS/générique qui noierait le message) si `ANDROID_TV_HOST` sort du LAN. Et `send_key()` exige une confirmation à l'écran (`confirm.py`, même mécanique que Drive/Gmail/HA) avant STOP, qui coupe ce que quelqu'un regarde peut-être — la navigation et les autres touches média restent immédiates. | ▲ |
...
| # | Feature | État | Effort |
|---|---|---|---|
| ~~C10~~ | ~~« Envoie ce fichier sur mon téléphone / la télé »~~ **Résolu** | Deux outils écrits : `tv_push_file` — `android_tv.push_file()` (nouveau, `device.push` symétrique du `device.pull` déjà utilisé par `screenshot()`) copie le fichier dans `/sdcard/Download/` de la télé et déclenche un scan média. `send_file` (cas général) — `brain/files.py` (nouveau) copie le fichier dans `data/uploads/<id-uuid4>/`, `GET /api/files/{id}/{filename}` (`brain/server.py`) le sert sans auth Bearer (le lien lui-même fait office de secret, comme les callbacks OAuth — ajouté à la liste blanche du middleware) ; l'outil émet une carte `files` (réutilise le renderer existant, aucun changement front) avec le lien. Comme les cartes diffusent déjà en direct vers toutes les Consoles ouvertes, le lien apparaît sur le téléphone, désormais aussi via notification push (C12) si sa Console est fermée. Testé : import des modules, `store_path()`/`resolve()` vérifiés avec un vrai fichier temporaire (dépôt, relecture, id invalide → `None`, id inconnu → `None`). **Non testé en conditions réelles** : le push ADB vers une vraie télé (nécessite le stick physique, hors de cet environnement). | ▲▲ |
...
| ~~C12~~ | ~~Notifications quand la Console est fermée~~ **Résolu** | `brain/push.py` (nouveau) : paire de clés VAPID auto-générée au premier lancement (`data/vapid.json`), abonnements stockés dans `data/push_subscriptions.json`. `brain/cards.py::emit` pousse désormais un Web Push (via `pywebpush`) pour les cartes `timer` et `proactive` — les deux seuls types qui déclenchaient déjà `new Notification(...)` côté `Hud.jsx` (onglet ouvert), en plus de la diffusion `/ws/cards` existante, inchangée. Trois routes (`GET /api/push/vapid-public-key`, `POST /api/push/subscribe`, `POST /api/push/unsubscribe`) derrière l'auth Console habituelle. Côté web : `web/public/sw.js` (service worker minimal, sans cache d'assets — voir `index.html`) + `web/src/lib/usePush.js::subscribeToPush()`, appelé depuis `System.jsx::TimersSection` au même geste utilisateur que `Notification.requestPermission()` (jamais au chargement de la page). Abonnements périmés (404/410) auto-nettoyés au prochain envoi. Testé : génération de clés, aller-retour `subscribe`/`unsubscribe`/`emit` via `TestClient` (bout en bout jusqu'à l'appel `pywebpush`, qui échoue comme attendu sur un faux `p256dh`), build + tests front (`npm run build`, `npm test`) verts. **Non testé en conditions réelles** : réception effective d'une notification sur un téléphone avec l'onglet fermé (nécessite HTTPS — voir note ci-dessous — et un vrai abonnement de navigateur, hors de cet environnement). | ▲▲ |
| ~~C13~~ | ~~Continuité de conversation visible~~ **Résolu, élargi en arbitrage audio temps réel** | Reformulé avec Quentin : au-delà d'un badge passif, l'enjeu réel est qu'ouvrir la Console sur le téléphone pendant que le PC fixe répond encore (ou l'inverse) ne fasse pas parler les deux appareils en même temps sur la même conversation partagée. `brain/presence.py` (nouveau) : état en mémoire « quel appareil a la main », dernier appareil à avoir entendu « Jarvis » (ou reçu une commande tapée) gagne — volontairement simple, pas de fenêtre de debounce, suffisant pour un usage personnel mono-utilisateur. `POST /api/presence/activate` / `GET /api/presence` (`brain/server.py`, exemptés du bearer Console comme `/api/health` — rien de sensible en jeu, juste un libellé), diffusés en direct à toutes les Consoles via `cards.presence_changed()` (même canal `/ws/cards` que les cartes et `exchange`). Côté PC : `agents/desktop/brain/presence_client.py` (nouveau) — `activate()` appelé à chaque question (`runtime.py::_handle_question`), fire-and-forget dans un thread pour ne jamais ralentir la boucle vocale ; `is_active()` consulté juste avant chaque `tts.speak()` (dans `say()`, `_speak_stream()` et la branche pilotage PC de `_answer_with_brain()`) — reste silencieux si un autre appareil a pris la main depuis, le texte s'affiche quand même. Fail-open total si `BRAIN_ENABLED=0` ou le brain injoignable (aucun changement de comportement sans opt-in, même principe que `remote_chat.py`). Côté web : `usePresence.js` (nouveau, id d'appareil stable en localStorage) branché dans `VoiceContext.jsx` (une seule instance pour toute l'app, comme la voix elle-même) — `activate()` appelé dès la détection du mot d'éveil (`useVoice.js::handleDetected`), `isActive()` consulté juste avant de jouer l'audio de synthèse (`drainQueue`). Badge visible « 🔊 réponse sur : … » ajouté à la fois dans `Console.jsx` (VoiceBar) et `Hud.jsx` quand un autre appareil a pris la main — satisfait la demande originale de C13 (indicateur de provenance) comme effet de bord de l'arbitrage, sans dupliquer le mécanisme. **Testé** : suite Python complète (84 tests) verte, `npm run build` et `npm test` (10 tests) verts, aller-retour `POST activate` → `GET` vérifié en conditions réelles via `TestClient` (dernier appareil activé gagne, 400 propre si `device` manquant, endpoints bien exemptés du bearer). **Non testé en conditions réelles** : le scénario physique complet (parler au PC puis ouvrir le téléphone en vrai) — nécessite le brain lancé + un vrai micro sur les deux appareils, hors de cet environnement. | ▲▲ |
